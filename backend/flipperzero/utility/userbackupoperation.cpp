...
void UserBackupOperation::readFiles()
{
    ...
    // No regular files means the loop below never reaches the RegularFile
    // branch, so nothing would ever call advanceOperationState() and the
    // operation would hang forever waiting for a read that was never issued.
    // An empty (or all-directories) source tree is a valid, if unusual, state
    // -- e.g. right after a factory-fresh /int -- not an error.
    if(numFiles == 0) {
        advanceOperationState();
        return;
    }

    for(const auto &fileInfo: std::as_const(m_fileList)) {
