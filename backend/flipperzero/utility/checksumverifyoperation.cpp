...
{
    for(const auto &url : std::as_const(m_urlsToCheck)) {
...
void ChecksumVerifyOperation::verifyMd5Sums()
{
    ...
    for(const auto &entry : std::as_const(m_flatFileList)) {
        ...
        // The default 30s suits a script or a keyfile, but a firmware's
        // resources.tar.gz runs to megabytes and the Flipper hashes it slowly.
        // The larger forks took longer than that, and the whole install failed
        // on a file that had transferred perfectly.
        operation->setTimeout(180000);
