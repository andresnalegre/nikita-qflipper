...
void Cli::initParser()
{
    ...
    m_parser.addPositionalArgument(QStringLiteral("reboot"), QStringLiteral("Reboot the device"), QStringLiteral("reboot,"));
    m_parser.addPositionalArgument(QStringLiteral("format"), QStringLiteral("Format External Storage (SD card)"), QStringLiteral("format,"));
...
void Cli::processArguments()
{
    ...
    if(args.isEmpty()) {
        ...
        beginWipe();
    } else if(args.startsWith(QStringLiteral("reboot"))) {
        beginReboot();
    } else if(args.startsWith(QStringLiteral("format"))) {
        beginFormat();
...
void Cli::beginReboot()
{
    verifyArgumentCount(1);
    qCInfo(LOG_CLI) << "Rebooting the device...";
    m_pendingOperation = Reboot;
}

void Cli::beginFormat()
{
    verifyArgumentCount(1);
    qCInfo(LOG_CLI) << "Formatting external storage...";
    m_pendingOperation = Format;
}
...
void Cli::startPendingOperation()
{
    ...
    if(m_pendingOperation == DefaultAction) {
        ...
        // Not implemented yet
    } else if(m_pendingOperation == Reboot) {
        m_backend.rebootDevice();
    } else if(m_pendingOperation == Format) {
        m_backend.formatExternalStorage();
