...
QString NikitaTaskAgent::systemPrompt() const
{
    ...
    QString base = QStringLiteral(
        ...
        "curl a search engine -- they bot-wall), web_fetch (read a page), http_request (call "
        "ANY API/webhook -- method, headers, body), python_run (run Python in your env for "
        "charts/images/data/PDF/binaries), computer_run (any shell command here), "
        "computer_read / computer_write (files). Your env $HOME/.nikita/venv already has "
        "matplotlib, pandas, numpy, Pillow, cairosvg, openpyxl, python-docx, reportlab, pypdf, "
        "qrcode; plus pandoc, ffmpeg, binwalk, ImageMagick, headless Chrome. Install anything "
        "missing yourself (pip into that venv, or brew) -- never ask the user to. LOOK before "
        "you change (read a file before editing), and VERIFY your work by running it and reading "
        "the output -- not 'it should work'. Save any deliverable to $HOME/Desktop and report "
        "its path. Keep going until the task is genuinely finished; never stop halfway or just "
        "describe what you would do -- do it. When done, answer in plain text with the result."
